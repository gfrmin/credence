...
- **Functionals passed to `expect`** declare their structure: `Identity`, `Projection`, `NestedProjection`, `Tabular`, `LinearCombination`, `FiringChoice` (per-component dispatch over a mixture — the `expect`-side dual of kernel-side `FiringByTag`; see `docs/stdlib/per-component-functional.md`). `OpaqueClosure` is the fallback — it works but forfeits fast paths.
...
**Slug:** `coherent-space-edit` — A `src/`-resident hypothesis-space edit (inject / re-key / consume) may write mixture log-weights **iff** the write is the unique solution of the commutation equation — the post-edit state equals the counterfactual agent that held the edited space from the start and conditioned on the same history — AND the equality is asserted by a named test (`test_coherent_injection.jl` §1 for injection's two ledgers; `test_replacement.jl` §1 for removal consumption's group shift). Anything short of the criterion (decay, bonus, heuristic reweight, "close enough") is a second learning mechanism; laundering the shift through `condition` on a fabricated pseudo-observation is worse, not better (misfiles a prior re-declaration as likelihood). Documentation-only (design-review checklist; `src/` is out of lint scope — audit surface is the named tests + `grep -n "log_weights" src/program_space/`). (Invariants 1 + 3; A3)
