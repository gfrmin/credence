...
The operational consequence is that **events are first-class bearers of probability**, not derived subsets of a measurable space. P(A) is declared directly for a declared proposition A, and conditional probability P(· | A) is a primitive alongside it. Parametric Bayesian update — the familiar `P(θ | o) ∝ P(o | θ) P(θ)` — and event-form conditioning are **peer primary primitives** at the Prevision level (Move 7 §5.1 Option B); neither derives from the other.

In the code this is realised by a fourth first-class type, `Event`, and two peer-primary forms of `condition`:

- **Parametric**: `condition(p, k, obs)` — takes a kernel and an observation. The familiar Bayesian update; every existing consumer works verbatim.
- **Event-form**: `condition(p, e::Event)` — takes a declared event directly.

Both are primary primitives at the Prevision level. They are **provably equivalent on deterministic events** (Di Lavore, Román, Sobociński, *Partial Markov Categories*, arXiv:2502.03477, Proposition 4.9: "Pearl's and Jeffrey's updates coincide for deterministic observations"). For stochastic observation kernels on continuous observation spaces — the common case for real-valued likelihoods — the event `{k emits exactly obs}` has Lebesgue measure zero, so reducing the parametric form to event-form in that regime requires **disintegration**. Disintegration is out of scope for Posture 3 and flagged as a future axiom extension; the peer-primary framing is the honest alternative to shipping sugar over a mathematically undefined reduction.

Events are declared structure per Invariant 2: `TagSet(space, tags)`, `FeatureEquals(space, name, value)`, `FeatureInterval(space, name, lo, hi)`, plus Boolean composition via `Conjunction` / `Disjunction` / `Complement`. Each carries its data in typed fields; `indicator_kernel` is the mechanical bridge from the event layer to the axiom layer, used by the event-form expansion on deterministic-event Measure dispatches (Posture 2 gate-2; Move 7 preserves the behaviour while elevating the dispatch surface to the Prevision level).
...
> **Margin (grid fineness is already priced — for anyone refining a discrete partition).** A threshold or grid-point drawn from an $n$-point grid costs $\log_2 n$ bits in $|\text{program}|$ to say *which* point — so description length **already rises with grid fineness** under this prior, on the existing program axis ($\lambda = \ln 2$). A finer partition is *not* free; refining a grid does not need, and must not introduce, a separate fineness axis. The implementation's `expr_complexity(GTExpr) = 1` is a deliberate *under-count* of this $\log_2 n$ term — exact and harmless while the grid is fixed (the term is constant and cancels under renormalisation), an under-count the moment the grid size changes. **Two equivalent routes realise this fineness-Occam:** charge it in the *prior* (correct `|program|` to include $\log_2 n$), or let the *predictive marginal likelihood* charge it — Bayesian Occam is the parameter integration, so a finer grid wins on the predictive only if its evidence beats the coarser grid's, and a split that merely fits noise does not improve the predictive. The two routes are interchangeable **only for the predictive (marginal) likelihood**; a point-estimate or max-likelihood score has no Occam and would chase unbounded refinement, at which point the prior route becomes mandatory. An explorer that values refinement by predictive-log-loss gain may therefore leave the prior fineness-blind soundly — *because* it is on the predictive route, not despite it. (Exploration-budget Move 3, Q1(b)≡Q3(a).)
...
### 6.9 The consumption boundary: the wire is the only public surface

The brain/body split has a packaging consequence. The engine (the brain machinery: DSL core + the axiom-constrained functions) is consumed by external applications across exactly one sanctioned surface: the **skin wire** (the JSON-RPC protocol of `apps/skin/`, versioned independently of the engine — see `apps/skin/protocol.md`). Across that wire, Measures stay server-side as opaque state IDs; a consumer never holds a measure, so it physically cannot read or do arithmetic on probability weights. The wire boundary therefore enforces Invariant 1's spatial face *by construction* for any application on the far side. External applications declare their domain as **data** — inline BDSL spaces/kernels/priors (`dsl_sources`) — and call the canalised primitives; they carry no probabilistic Julia of their own. The official thin client is `credence-skin-client` (pure JSON-RPC plumbing, zero engine code); the versioned engine artifact is the `credence-skin` image. This is the decoupling commitment: *credence is the engine; an application gets data and a thin body, never a brain, and the engine does not ship the means for an external application to host one.*

**The co-released-image exception.** In-process embedding — loading the engine into the application's own process via `juliacall` (the `credence` / `credence-agents` packages) — *is* hosting a brain, and is forbidden as a published library that external applications import. The two are demoted to engine-repo-internal (`Private :: Do Not Upload`). The single exception: code **co-released inside an engine-repo image** may embed in-process, because there the brain is built and versioned together with the engine, with no application-side embedding — `credence-proxy` (the `credence_router` gateway) is the standing instance, a product image shipped from this repo, not a library others depend on. (The former `credence-pi-daemon` exercised this exception until the *decouple* Move 5 extracted it to its own repo, `credence-openclaw`, as a pure wire consumer — proof the boundary holds for a fully external app: it pins the `credence-skin` image and drives the verbs, carrying no probabilistic code.) See `docs/decouple/master-plan.md`.
