...
let
    ...
    check("5-feature model has 32 structures", length(model.structures) == 32,
          "got $(length(model.structures))")
    # all-edges structure (the last, mask=11111) has 8·4·9·4·4 = 4608 cells.
    check("all-edges structure has 4608 cells",
          length(model.cell_tag[end]) == 8*4*9*4*4, "got $(length(model.cell_tag[end]))")
    top = build_prior(model)
