...
def test_categorical_prior():
    ...
    p = Prevision.categorical(s, [0.3, 0.7])
    w = p.weights()
    assert abs(w[0] - 0.3) < 1e-6
    assert abs(w[1] - 0.7) < 1e-6


def test_beta_prevision():
    p = Prevision.beta(8.0, 2.0)
    assert abs(p.mean() - 0.8) < 1e-10  # credence-lint: allow — precedent:test-oracle — Beta(8,2) mean = 8/10 = 0.8
    expected_var = 8 * 2 / (10**2 * 11)
    assert abs(p.variance() - expected_var) < 1e-10  # credence-lint: allow — precedent:test-oracle — Beta variance αβ/((α+β)²(α+β+1))


def test_gaussian_prevision():
    p = Prevision.gaussian(5.0, 2.0)
    assert abs(p.mean() - 5.0) < 1e-10  # credence-lint: allow — precedent:test-oracle — Gaussian(5, 2) mean = 5
...
def test_expect_identity():
    ...
    p = Prevision.uniform(s)
    result = expect(p, lambda h: float(h))
...
def test_product_prevision():
    p1 = Prevision.beta(2.0, 3.0)
    p2 = Prevision.beta(5.0, 1.0)
    prod = Prevision.product([p1, p2])
    sample = draw(prod)
    assert len(sample) == 2
...
def test_mixture_prevision():
    s = Space.finite([0.0, 1.0])
    c1 = Prevision.categorical(s, [0.9, 0.1])
    c2 = Prevision.categorical(s, [0.1, 0.9])
    mix = Prevision.mixture([c1, c2], [0.5, 0.5])
    w = mix.weights()
    assert len(w) == 2
...
def test_load_dsl():
    ...
    assert "H" in env
    assert "prior" in env
    assert isinstance(env["prior"], Prevision)
    w = env["prior"].weights()
    assert len(w) == 2
