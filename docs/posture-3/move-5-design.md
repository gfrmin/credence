...
**Downstream consequences for Move 6.** Move 6 is the particle / quadrature / enumeration refactor — adds `ParticlePrevision` and `QuadraturePrevision` as Prevision subtypes. The Socratic resolution above has direct downstream consequences:

- **If Move 5 ships `ExchangeablePrevision` (Option A, the committed position):** Move 6 inherits `ParticlePrevision` as a fresh type with no back-compatibility debt. Move 6's design-doc grep can confirm no production consumer constructs `CategoricalMeasure(Finite(samples), log_weights)` outside the particle path (currently `src/ontology.jl:660-684,848-860`). Clean additive refactor.
...
**Recommendation: A.** The Measure-surface API — `condition(m::Measure, …)` for each Measure subtype — is a public contract. Deleting a method that is part of that contract requires a deprecation cycle (announce → warn → remove), not a mid-refactor silent drop, even when the grep confirms no current consumer uses it. Delegation costs a one-line stub and preserves the contract; deletion buys nothing the refactor needs, at the cost of a public-API breaking change for a hypothetical consumer that the grep cannot rule out for ever.
...
### 5.4 (substantive) `_dispatch_path(p::MixturePrevision, k)` return shape

Move 4's `_dispatch_path(p::Prevision, k) → Symbol` commits to a state-free query-only hook returning `:conjugate` or `:particle`. Move 5's MixturePrevision has more structure: each component has its own dispatch path, and they can disagree (e.g. 2 of 3 route conjugate, 1 falls through to particle). Three candidate shapes:

- **Option (i) single-Symbol rollup.** `_dispatch_path(p::MixturePrevision, k)` returns `:conjugate` iff every component routes conjugate; else `:mixed`. (`:particle` is reserved for when no component routes conjugate, so `:mixed` is the honest label for partial-coverage.)
- **Option (ii) `Vector{Symbol}` per component.** Returns a vector of length `length(p.components)` with the per-component Symbol.
- **Option (iii) single-Symbol with optional component index.** Keep the Move 4 signature; extend with an optional kwarg `_dispatch_path(p, k; component=i)` that returns the i-th component's path, and `component=nothing` (default) returns the rollup.

**Recommendation: (i) single-Symbol rollup.** Three reasons: (1) preserves the Move 4 return-type contract (always a `Symbol`) — test code that pattern-matches on the return value of `_dispatch_path` at any Prevision type doesn't need a per-type case analysis. (2) The halt-the-line failure mode §7 names — "silent registry-or-routing miss" — surfaces in the rollup: a single component that silently falls through to particle flips `:conjugate` to `:mixed` for the whole mixture, firing the test without requiring per-component inspection. (3) Per-component visibility remains available via `_dispatch_path(p.components[i], k)` — the component previsions are themselves Previsions, and Move 4's hook dispatches on them. Tests construct components first and query `_dispatch_path` on each before bundling into the mixture; the mixture-level hook summarises.

**Invitation to argue.** Option (ii) becomes correct if the §3 verification invariants expand to assert the *specific multiset* of per-component paths rather than the rollup alone — e.g. "exactly 2 components route to `:conjugate` via BetaBernoulli, exactly 1 via Flat, zero via particle." Moves 6 (particle refactor) or 8 (cosmetic adaptation) could legitimately need that granularity. Option (iii) is most composable but adds a kwarg surface for a case the recommended approach already covers. Commit to (i); promote to (ii) if a Move 6/8 test case cannot be expressed in the rollup form.
...
**Risk R3 (low, scoped): `ExchangeablePrevision.decompose` ships with known-limited test coverage; real-shape correctness is the email-agent follow-up's load-bearing check.** Per §5.1's Option-A posture, `decompose` lands at Move 5 without its production consumer. The Move 5 test corpus (`test/test_prevision_mixture.jl`) exercises the Dirichlet case explicitly — construct `ExchangeablePrevision(Finite([1,2,3]), DirichletPrevision([α₁, α₂, α₃]))`, call `decompose`, assert the returned `MixturePrevision` has three components with log-weights matching the Dirichlet marginal at each category (closed-form, `rtol=1e-12`). This is the simplest representation-theorem shape: one ergodic component per category. It does *not* cover non-trivial exchangeable structure (mixtures of Dirichlets, hierarchical priors, the 22-program email-agent shape). A `decompose` bug that only manifests on those shapes survives Move 5 and is surfaced at the email-agent migration follow-up PR when the real consumer lands.

This is a deliberate scoping, not an oversight. The alternatives are (a) expand `test/test_prevision_mixture.jl` to exercise synthetic hierarchical / nested-mixture exchangeable priors, or (b) ship `ExchangeablePrevision` alongside its consumer (Option B from §5.1). Option (a) adds synthetic test shapes whose correctness reduces to "does the test match the author's mental model of the representation theorem on a prior no consumer uses" — a weak check that invites confirmation bias. Option (b) is the §5.1 fallback posture; it defers real correctness to the consumer PR, which is where correctness lives anyway. Move 5 commits to Option A with the acknowledgement that `decompose` ships with simplest-case coverage and the email-agent follow-up PR carries the hierarchical / multi-component correctness burden.

*Mitigation lives in the follow-up PR, not here.* The email-agent migration PR's design doc must name `decompose` correctness as its own R1 (not inherited as stale scaffolding) and extend `test/test_prevision_mixture.jl` with the 22-program shape as a test case. If this Move 5 PR merges and the email-agent migration doesn't land within ~2 moves, re-open Move 5's §5.1 as a revisit: shipped-without-real-consumer for more than ~2 moves is the point at which the deferred-work-debt argument for Option B outweighs the paper-as-citable-artifact argument for Option A.
...
**Risk R5 (low): Move 4's `_dispatch_path` observability must report correctly under the new dispatch path.** Move 5's component routing goes through the same `maybe_conjugate` + `update` path Move 4 established. The mixture-level return shape is committed in §5.4: single-Symbol rollup, `:conjugate` iff every component routes conjugate, `:mixed` otherwise. *Caught by:* the §3 verification invariants assert `_dispatch_path(mp, k) == :conjugate` on the worked example (3 components, all route conjugate via BetaBernoulli / Flat); a routing bug that leaves any component on the particle fallback flips the rollup to `:mixed` and the assertion fires. Per-component drilldown in tests goes through `_dispatch_path(mp.components[i], k)` using Move 4's per-Prevision hook.
