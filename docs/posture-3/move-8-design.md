...
- `docs/posture-3/paper-draft.md` — comprehensive completion per §1 acceptance criterion. Six sections:
  ...
  - **§3 Comparison** — four entries (§3.1 Staton: measure-categorical PPL semantics; §3.2 Jacobs: effectus theory; §3.3 Hakaru and disintegration-first treatments; §3.4 MonadBayes and score-primitive monads). Proportionality per §5.2 (Option B, proportional length). *Drafter note:* first pass writes one paragraph per entry; second pass expands the entries where the "shared concerns / differences / relation to our framework" exposition genuinely has more to say (expected: Staton, MonadBayes, Hakaru-on-disintegration); keeps the shorter entries (expected: Jacobs) at one paragraph. If on the second pass all four expand equally, Option A is surfacing as the honest shape — flag for review.
...
**Invitation to argue.** Option C (no bridge, no replacement) becomes correct if review surfaces the bridge paragraph as hedging — a reader who thinks "why does the doc need to defend §1.0 / §1.1 coexistence?" might conclude the coexistence is problematic when the audit found it isn't. Collapse to C on that specific review feedback; otherwise stay at B. (If §1.3 and §1.5 phrasing tweaks expand beyond one-sentence diffs, that's a §1.b scope decision — the tweaks get deferred or lifted to a post-Move-8 follow-up — not a §5.1 reopening.)
...
**Invitation to argue.** Option A (one paragraph each) becomes correct for venues where submission norms favour equal-weight comparison sections — some AISTATS tracks and NeurIPS workshop formats explicitly prefer that shape. If the paper's target venue specifies that norm, A is defensible on venue-compliance grounds rather than reader-engagement grounds. Venue targeting is a paper-PR decision rather than a design-doc decision; default to B, lift to A if the venue's CFP calls it out.
...
1. **Reproducibility and orthogonality of failure modes.** A §4 SHA pin exists specifically to let a reader reproduce the paper's operational claims: check out the pinned SHA, run the test suite, observe the canonical validation. The Move-8-merge SHA uniquely provides this because body-work (Gmail connection, Telegram connection, credence-proxy iteration) introduces *orthogonal* failure modes — network flakes, external-API rate limits, Telegram API surface changes, credential expiry — that are irrelevant to the reconstruction's claims. Pinning a later SHA forces the reader to disentangle reconstruction-relevant test signal from body-work noise; pinning Move-8-merge makes the check clean.

2. **Paper-to-artifact claim validation.** Every §2 worked example in the paper references a specific Move (Move 4's registry, Move 5's MixturePrevision, Move 6's ParticlePrevision, Move 7's event-form primary). Each claim is validated at the Move-8-merge SHA. Post-Move-8 body-work may add features but doesn't change reconstruction-level claims; the paper-to-artifact relationship stays tight when the pin is at reconstruction-completion.
...
**Risk R3 (low): Reference-list arXiv identifier drift.** Paper references include arXiv identifiers (DLRS 2502.03477 verified at Move 7; others to be audited at Move 8). *Mitigation — method named:* for each arXiv entry in the references list, use `web_fetch` on `https://arxiv.org/abs/<ID>`; confirm the title and authors displayed on the arXiv abstract page match the references-list entry. For non-arXiv references (books, peer-reviewed journal papers with DOIs, conference proceedings), visual inspection against a reputable source (publisher page, Google Scholar record, ACM/IEEE/Springer catalogue) is sufficient. Any mismatch is halt-the-line at the paper PR. This is reproducible discipline: a reviewer can re-run the same verification and arrive at the same yes/no answer, rather than trusting "verified" as a checkbox.
