...
The contribution is operational, not just philosophical. The reconstruction passes the existing test suite under a three-strata discipline: bit-exact (`==`) for closed-form conjugate updates (Stratum 1 unit equivalence) and for particle paths under deterministic seeding (Stratum 2); `rtol=1e-12` for numerically-sensitive closed forms where arithmetic reassociation is legitimate; `rtol=1e-10` as the Stratum-3 end-to-end floor. It opens three new pieces of expressive power that the measure-theoretic implementation could not state: (i) the `ConjugatePrevision` registry as a single source of truth for fast paths, (ii) `ExchangeablePrevision` as a first-class type that gives the email agent's parallel-program belief its true category-theoretic shape, and (iii) `condition` on events as the primitive form, with parametric Bayes update derived as one instance.
...
function _condition_particle(m::Measure, k::Kernel, obs; n_particles=1000, seed=0)
    ...
    # Pass-by-reference facade: the second argument is pp (the typed
    # Prevision), not a bare log_weights Vector — this is the Move 3
    # getproperty shield pattern extended to the ParticlePrevision
    # case. The shield forwards .logw → pp.log_weights and
    # .space.values → pp.samples by reference. No defensive copy;
    # downstream consumers read through the shield to the underlying
    # Prevision's fields and the typed carrier survives past the wrap.
