...
**Risk 1: `condition` dispatch gap for scalar Previsions.** If the skin stores a `BetaPrevision` and calls `condition(::BetaPrevision, k, obs)`, no method exists. Mitigation: §5.2 option (c) — scalar beliefs stay as Measures in the skin. Only `AgentState.belief` becomes `MixturePrevision`.
...
2. **Does this move hold a Prevision inside a Measure, or a Measure inside a Prevision?** `AgentState.belief` changes from `MixtureMeasure` (Measure wrapping Prevision) to `MixturePrevision` (pure Prevision). This removes one layer of Measure-wrapping-Prevision, directionally correct. The skin's scalar beliefs (standalone `BetaMeasure`, `GaussianMeasure`, etc. in the state registry) retain the Measure wrapper per §5.2 option (c) — pending scalar `condition` methods. This is explicit and bounded.
