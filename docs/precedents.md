...
### Averaging a model posterior vs. collapsing it
**Slug:** `average-not-collapse`.
**Rule.** A posterior over models, likelihood families, or programs — a `MixturePrevision` over structures/families/programs produced by BMA — is *carried*. Decisions marginalise over it: `a* = argmax_a expect(mixture, u_a)`, the expectation averaging utility over model uncertainty and the `argmax` ranging only over *actions*. The model index is integrated out, never selected.
**Illegal.** Collapsing the posterior to its most-probable component — `m* = argmax_m weights(mixture)` — and then deciding as if `m*` were certain. That discards model uncertainty and installs a second decision mechanism (max-posterior selection) outside `optimise`. "Pick the best family", "the winning program", "select the MAP structure" are this error by other names. (This is the `compute-on-weights` violation specialised to model selection, but it merits its own slug because the mistake reads as natural inference rather than as weight arithmetic.)
**Legal collapse is itself a decision under utility.** Committing to a single model is sometimes warranted — a deployment that can carry only one likelihood, or where maintaining the mixture has a real compute/latency cost. When it is, *which* model to keep is an `optimise`: `argmax_m EU(commit to m)` under the model-carrying-cost utility, **not** `argmax_m P(m | data)`. The two coincide only when the carrying-cost utility is degenerate. Collapse done correctly flows through the one decision mechanism; it needs no escape hatch because it is ordinary EU-max.
**Follows from Invariant 1, and A2/A3 jointly.** EU-maximisation marginalises over the belief, and the belief includes model uncertainty; selecting the max-posterior model to drive behaviour is the parallel decision mechanism A4 exists to forbid. There is no escape hatch for the illegal form. The slug is for cross-referencing the distinction at any site that builds a mixture over structures, families, or programs — `structure_bma.jl`, family-BMA, and the program-space belief are the live examples.
...
**Corollary — carrier-bound ops are Measure-resident (measure-as-view Phase 3, #163).** "Thin facades" governs *carrier-free* ops only. Because Measure = Prevision **+ carrier**, the rule entails carrier-free ops are Prevision-primary — NOT that *every* op is. An op that reads the carrier's data values (`condition(m::CategoricalMeasure)` reading `m.space.values`; the `factor_selector` expansion; a predictive evaluated at the atoms) is legitimately Measure-resident; threading the carrier into the Prevision to force it Prevision-primary would bind the carrier into the Prevision and dissolve the carrier-free core the view exists *over* (self-defeating). A Prevision-level `MethodError` for such an op is the boundary asserting itself, not a gap — the measure-level analogue of `condition` refusing to run without a likelihood. This is an entailment of the rule, not a new commitment.
...
### Coherent hypothesis-space edits writing mixture weights
**Slug:** `coherent-space-edit`.
**Context.** Editing the hypothesis space mid-run — injecting enumerated newcomers (coherent injection, #187), consuming a grammar into its cleaned successor (removal consumption, the replacement move) — requires log-weight arithmetic outside `condition`: injection restores the cross-group normalisation constants its replay discarded (the two ledgers); replacement shifts the consumed group by `Δ = λ·(complexity(g) − complexity(g′))`. Read literally, "no other function may modify a measure's weights" forbids both; read purposively, the letter exists to prevent *second learning mechanisms* — weight writes that encode evidence or preference outside Bayes.
**The criterion.** A `src/`-resident hypothesis-space edit (inject / re-key / consume) may write mixture log-weights **iff** the write is the **unique solution of the commutation equation** — the state after the edit equals, exactly, the state of the counterfactual agent that held the edited space from the start and conditioned on the same history — **and** that equality is asserted by a named test. The write then adds no information Bayes would not have produced: it is bookkeeping for *when* a hypothesis was represented, never *what the evidence said*. Anything short of the criterion — a decay, a bonus, a heuristic reweight, a shift "close enough" to coherent — is a second learning mechanism, full stop.
**Legal.** `add_programs_to_state!`'s two-ledger de-normalisation (commutation asserted by `test_coherent_injection.jl` §1); `replace_grammar_in_state!`'s group shift `lw[i ∈ G] += Δ` (commutation asserted by `test_replacement.jl` §1 — exact for the full history because every likelihood term cancels identically).
**Illegal.** Routing the same shift through `condition` on a *fabricated* pseudo-observation (it misfiles a prior re-declaration as likelihood, corrupting every downstream evidence read — the T-1.3 violation); any weight write whose value is chosen rather than derived; a derived write without its named equality test.
**Named-test requirement.** Each sanctioned write's docstring names the test that asserts its commutation equality, and the test references the write (executable-documentation discipline). Breaking either breaks both.
**No pragma.** The writes live in `src/` (out of lint scope); this is a design-review checklist for space-edit additions. The audit surface is the named tests plus `grep -n "log_weights" src/program_space/`.
**Follows from Invariants 1 + 3 and A3**: the commutation criterion is exactly "conditioning is the only learning" stated for a moving support — Bayes does not care when you thought of the hypothesis, so the edit that agrees with the counterfactual full-history posterior carries zero extra information (Invariant 1); the derivation lives once, in the edit function, with its test as witness (Invariant 3). Retroactively regularises #187's ledger arithmetic (previously resting on `stdlib-composition` by residence); prospectively governs add-consumption and any future space edit.
