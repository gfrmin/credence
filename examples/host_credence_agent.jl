...
update_reliability_fn = env[Symbol("update-reliability")]
...
for q in 1:50
    ...
    while !done && !isempty(available)
        ...
        elseif action_type == 0  # submit
            ...
            # Update reliability beliefs for queried tools
            for (t, resp) in tool_responses
                resp === nothing && continue
                correct = Float64(resp == true_answer)
                for c in 1:n_categories
                    rel_beliefs[t][c] = update_reliability_fn(rel_beliefs[t][c], correct)
                end
            end
