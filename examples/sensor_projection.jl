"""
    sensor_projection.jl — Sensor configuration and projection layer

Each grammar defines a SensorConfig: a set of channels that project the
environment's true state vector into the agent's perceptual space.
Different grammars can "see" different things.
...
`true_state` is a Vector{Float64} of length N_SOURCE_DIMS.
`temporal_state` is a Dict{Int, Vector{Vector{Float64}}} mapping entity_id
to past true_state vectors (for delta/windowed_mean transforms).
"""
