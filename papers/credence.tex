...
The computational asymmetry is stark: the \credence{} decision layer processes each question in $0.07$ seconds (closed-form Beta posterior updates and VOI computation), whilst the best LLM agent requires $5.65$ seconds---an $80\times$ difference driven entirely by LLM inference overhead for tool selection decisions that admit analytical solutions. \Cref{tab:stationary} reports wall-clock time per question for each agent.
