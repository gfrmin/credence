...
Our benchmark results (\S\ref{sec:results}) illustrate the cost-performance frontier: a local LLM with cost-awareness prompts and tool-calling still scores barely half the best zero-cost Bayesian agent, whilst a commercial LLM outperforms it only at non-trivial API cost. Natural language cannot express the quantitative tradeoff between information gain and query cost that VOI captures analytically. \citet{desabbata2024metareasoning} reach a complementary conclusion from the metareasoning perspective: LLMs benefit from explicit computational cost-benefit analysis rather than implicit prompt-level reasoning.
...
\paragraph{Scope of this paper.} The experiments here exercise Axioms~1--3 (conditioning, integration, EU-maximisation), which fully determine the tool-selection behaviour we evaluate. The complexity prior (Axiom~4) and the CIRL alignment commitment (Axiom~5) are framework-level commitments developed in the companion work; they do not act on the present benchmark, where the utility function is fixed and known and the hypothesis space is enumerated. In particular, the $\Beta(1,1)$ reliability prior introduced below is a per-tool conjugate object over a \emph{reliability parameter}---distinct from, and not in tension with, the Axiom-4 description-length prior over \emph{hypotheses}.
...
\paragraph{Non-stationarity.} Tool reliability genuinely drifts in deployment---commercial LLM behaviour, for instance, changes measurably over time \citep{chen2023chatgpt}. The core framework handles such distributional drift through standard Beta-Bernoulli updating: as a tool's reliability degrades, negative evidence accumulates and the posterior shifts downward naturally. No explicit change-detection mechanism is required---the posterior adapts continuously, with the rate of adaptation governed by the effective sample size (the sum $\alpha + \beta$).
...
\begin{itemize}
\item \textbf{Claude Haiku 4.5} (claude-haiku-4-5-20251001): a production commercial model---Anthropic's fast, low-cost tier (larger models such as Claude Sonnet and Opus sit above it and would score higher still). API cost: \$1/M input, \$5/M output tokens. We include it to show that \emph{even the cheapest paid tier} comprehensively outscores the \$0 family; it is a lower bound on what a paid LLM buys, not an upper bound.
...
\caption{\textbf{(A) The contingency law.} Horizon-aware VOI beats optimism-greedy by $+27$ when categories are \emph{given} (oracle) and loses by $15$ once they are \emph{inferred} (a $0.78$-accuracy classifier); the crossover is the contribution. Myopic VOI (faint) loses in both conditions. \textbf{(B) The cost--reward frontier} (tool cost vs gross reward; every agent is \$0 except Haiku). The Bayesian family owns the \emph{zero-dollar} frontier: the myopic-VOI agent is the frugal point that dominates the free local Llama (lower tool cost \emph{and} higher gross reward), greedy is the higher-scoring family point; Haiku, the lone dollar-paying agent, sits top-left but off the \$0 frontier.}
...
Horizon-aware VOI & $+134.2$ & $227.0$ & $63.6$ & $0.0$ & $1.86$ & $0$ \\
...
\Cref{tab:main} establishes the substrate. Every \emph{learning} policy buries the non-learning baselines (no-learning $+3.8 \approx$ chance-with-cost), and the Bayesian family occupies the cost-efficient region. The myopic-VOI agent dominates the free local Llama in the \emph{product order}---lower tool cost ($1.32 < 1.44$) \emph{and} higher gross reward ($176.5 > 151.5$, hence also higher score, $+110.4 > +79.3$)---making it the frugal frontier point, while optimism-greedy is the higher-scoring family member ($+149.6$) at a larger tool budget. Only paid Haiku ($+445.5$, $97.5\%$ accuracy) scores above the family, at \$3.24 per 1000 questions; the family answers $56$--$64\%$ correctly at \$0. We state the accuracy subtraction plainly---the claim is cost-efficiency, not parity.
...
\begin{table}[t]
\centering
\caption{The decomposition: score by action policy $\times$ category condition (same Beta-Bernoulli substrate throughout). The winner flips between conditions---this crossover is the contingency law. Oracle reference points: the exact prior-optimal decoupled policy scores $211.8$; the deployed depth-1 horizon agent realises $216.8$ (it slightly exceeds the prior-optimal policy because the true reliabilities are extreme, so the Beta$(1,1)$-optimal policy is not optimal on them); the only genuine ceiling is the known-$\theta$ upper bound, $306.2$.}
\label{tab:decomp}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
...
\paragraph{Exploration is fixable---with given categories.} Myopic VOI under-explores (\S\ref{sec:percat}) and loses to greedy in both conditions ($-25.7$ oracle, $-39.2$ fair). Horizon-aware VOI fixes this: valuing information over the remaining same-category questions (pure EU maximisation---no $\varepsilon$-greedy, no bonus) lifts the oracle score to $+216.8$, \textbf{beating greedy by $+27$}. A \emph{depth-1} lookahead already captures this gain (the depth sweep gives $217.6$), comparable to the exact prior-optimal decoupled policy ($211.8$) and far below the known-$\theta$ ceiling ($306.2$)---so the win needs no deep search. Depth-1 slightly exceeds the prior-optimal $211.8$ because the true reliabilities are extreme, which makes the Beta$(1,1)$-optimal policy suboptimal on them.

\paragraph{But the advantage is contingent on attribution quality.} Under inferred categories every exploratory query injects misattribution noise into per-category reliability learning, and the advantage evaporates: horizon-aware VOI is the \emph{most} inference-sensitive policy ($-82.6$). Greedy does not merely edge it out---horizon-aware VOI is Pareto-\emph{dominated} in the fair condition (higher tool cost, $1.86 > 1.62$, \emph{and} lower score, $134.2 < 149.6$), a stronger statement than the headline $-15$. To locate the cause, we isolate exploration---greedy's cost-blind submission plus always-on horizon-probing, a harder-exploring setup than the deployed depth-1 agent (which is why its gaps in \cref{tab:credit} exceed the deployed $-15$)---across the credit-assignment rule; the loss is a property of imperfect attribution, not of any one rule (\cref{tab:credit}).
...
Pillar~(i) holds in both conditions: reliability learning plus category inference buys the cost-efficient position. The product-order domination of the free Llama by the myopic-VOI agent is significant under a paired-seed bootstrap (95\% CI, $B = 10{,}000$ resamples): greedy${-}$Llama $+70.2$ $[+49.8, +91.2]$, myopic-VOI${-}$single-best $+66.2$ $[+44.4, +88.1]$, myopic-VOI${-}$Llama $+31.1$ $[+6.2, +56.2]$ (the cost-axis ordering $1.32 < 1.44$ that completes the product-order domination is by point estimate).

\paragraph{The cost-scalarisation honesty.} Domination depends on how the two cost axes (dollars, tool cost) are traded. Under a scalarised cost $\text{\$} + p \cdot (\text{tool cost})$, the myopic-VOI agent is undominated only in a narrow band $p \approx 0.001$--$0.005$ (dollars per unit tool cost): below it greedy dominates, above it Haiku. So the \emph{scalarised}-frontier claim is made at the family level (via greedy), while the myopic-VOI agent's robust claim is the product-order domination of Llama; Haiku is the paid-frontier point. We make both claims and overclaim neither.
...
The computational asymmetry is stark: the \credence{} decision layer processes each question in $0.016$ seconds (closed-form Beta posterior updates and VOI computation), whilst the commercial LLM requires $2.67$ seconds---a $167\times$ difference driven entirely by LLM inference overhead for tool selection decisions that admit analytical solutions.
...
Finally, the CIRL alignment axiom (\cref{sec:axioms}) opens a natural connection to the preference learning literature: the agent's beliefs about user preferences can be updated from implicit feedback (clicks, corrections, task completions) using the same conjugate machinery that tracks tool reliability. This would position \credence{} not merely as an agent framework but as an alignment-by-construction approach, in which safety properties derive from the alignment axiom rather than being imposed as constraints---a direction the present work sets up but, exercising only Axioms~1--3, does not yet test.
...
\bibitem[Wang et~al.(2024)]{wang2023llmagent}
