...
\paragraph{But the advantage is contingent on attribution quality.} Under inferred categories every exploratory query injects misattribution noise into per-category reliability learning, and the advantage evaporates: horizon-aware VOI is the \emph{most} inference-sensitive policy ($-82.6$). Greedy does not merely edge it out---horizon-aware VOI is Pareto-\emph{dominated} in the fair condition (higher tool cost, $1.77 > 1.62$, \emph{and} lower score, $134.2 < 149.6$), a stronger statement than the headline $-15$. To locate the cause, we isolate exploration---greedy's cost-blind submission plus always-on horizon-probing, a harder-exploring setup than the deployed depth-1 agent (which is why its gaps in \cref{tab:credit} exceed the deployed $-15$)---across the credit-assignment rule; the loss is a property of imperfect attribution, not of any one rule (\cref{tab:credit}).
