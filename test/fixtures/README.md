...
**Source SHA:** `af1fef51bb57bf94dfcedf7d7ee62a13b747f015` (master tip at Move 3 code PR opening; post-Move-3-design-doc merge).
**Capture date:** 2026-04-24.
**Size:** < 1 KB.
**Represents:** a `MixtureMeasure` of 3 `TaggedBetaMeasure` components with non-uniform posterior weights after synthetic `FiringByTag` conditioning. Wrapped in a `Dict(:__schema_version => 3, :belief => m, :note => ...)`; serialised via `Serialization.serialize`. The Measure structs inside are post-Move-2 layout: `TaggedBetaPrevision.beta::BetaPrevision` (not `::Any` / `BetaMeasure` as in the retired v1 fixture).

**Replaces:** `agent_state_v1.jls` (retired in Move 3 per `docs/posture-4/master-plan.md` §Move 3 and `decision-log.md` §Decision 3 — no compatibility with a non-existent user base).

**Construction script (verbatim, for provenance):**

```julia
# Capture script, run once at master SHA af1fef5.
push!(LOAD_PATH, "src")
using Credence
using Serialization
...
state = Dict(
    :__schema_version => 3,
    :belief => m,
    :note => "agent_state_v3 fixture; captured at master af1fef5 (post-Move-2); 3 TaggedBetaMeasure components with posterior after 2 pos + 1 neg FiringByTag(1,2) observations. Post-Move-2: TaggedBetaPrevision.beta is BetaPrevision (not BetaMeasure).",
)
...
- `state[:__schema_version] == 3`.
- `state[:belief]` is a `MixtureMeasure`.
...
- Component 3: `tag == 3`, `beta.alpha == 5.0`, `beta.beta == 2.0` (was Beta(5,2); unchanged — tag 3 not in `fires`).

Assertion tolerances: `==` on α/β (integer-accumulated).
...
### `email_agent_state_v3.jls`

**Source SHA:** `af1fef51bb57bf94dfcedf7d7ee62a13b747f015` (same as above).
**Capture date:** 2026-04-24.
**Size:** ~1 KB.
**Represents:** the email-agent persistence shape — `rel_beliefs` and `cov_beliefs` as single-component `MixtureMeasure`s of `ProductMeasure` of `BetaMeasure` (3 categories each), a `cat_belief` `CategoricalMeasure`, plus `total_score` and `total_cost` scalars. Serialised via the `save_state(path; rel_beliefs, cov_beliefs, cat_belief, total_score, total_cost)` public API — which writes a Dict with `:__schema_version => 3` plus the payload keys.

**Replaces:** `email_agent_state_v1.jls` (retired in Move 3).
...
Credence.save_state("test/fixtures/email_agent_state_v3.jls";
...
- `weights(state[:cat_belief])`: `[0.3, 0.5, 0.2]` (atol=1e-14 — reassociation-sensitive through `weights()`).
- `state[:total_score] == 42.5`.
- `state[:total_cost] == 0.123`.

Assertion tolerances: `==` on α/β; `atol=1e-14` on normalised weights; `==` on scalars.
